...
K2A2Recorder は、iOS のヘルスケアに保存された血圧データを確認し、最高血圧・最低血圧を追加・編集できる SwiftUI アプリです。
...
音声入力を使う場合は、マイクと音声認識の利用も許可してください。

## 使い方

- 右上の `+` ボタンから血圧入力パネルを開きます。
- 最高血圧・最低血圧を入力し、`保存` をタップすると HealthKit に保存されます。
- マイクボタンをタップして話すと、認識されたテキスト内の数値から先頭 2 つを最高血圧・最低血圧として入力します。
- 鉛筆アイコンが表示されている行は、このアプリで作成したデータです。行をタップすると編集できます。
- 編集時に最高血圧・最低血圧をどちらも空にして `更新` をタップすると、そのデータを削除します。
...
- 音声入力は `ja-JP` の `SFSpeechRecognizer` を使い、認識結果から数字を抽出して入力欄に反映します。
